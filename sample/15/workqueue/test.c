...
irqreturn_t workqueue_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    schedule_work(&work_queue);                 // 调度工作队列,注册,以便内核线程执行+
    wake_up_interruptible(&waitqueue_read);     // 唤醒
...
ssize_t workqueue_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
    ...
    schedule_work(&work_queue);                 // 注册工作队列
    interruptible_sleep_on(&waitqueue_read);    // 睡眠
...
{
    if(!request_irq(PRN_IRQ, workqueue_interrupt, IRQF_SHARED/*SA_INTERRUPT*/, DEV_NAME, NULL))
    {
        outb(PRN_IRQ_ENABLE_MASK, PRN_ADDRESS_CTRL);
    }

    try_module_get (THIS_MODULE);       // 模块使用的次数
    return 0;
}
